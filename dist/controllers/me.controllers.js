...
const pick = (src, keys) => keys.reduce((acc, k) => {
    if (src[k] !== undefined)
        acc[k] = src[k];
    return acc;
}, {});
export const getMe = asyncHandler(async (req, res, next) => {
    ...
    const ProfileModel = modelByRole[ctx.role];
...
export const updateMe = asyncHandler(async (req, res, next) => {
    ...
    const updated = await User.findByIdAndUpdate(ctx.sub, { $set: allowed }, { new: true })
        .select("-password -twoFactorAuthSecret -resetPasswordToken")
        .lean();
    if (!updated)
        return next(new AppError("USER_NOT_FOUND"));
    return ResponseHelper.success(res, { user: updated }, SUCCESS_CODES.OPERATION_SUCCESS);
});
export const updateMyProfile = asyncHandler(async (req, res, next) => {
    ...
    const updated = await ProfileModel.findOneAndUpdate({
        ...
        schoolId: ctx.schoolId,
        deletedAt: null,
    }, { $set: payload }, { new: true }).lean();
    if (!updated)
        return next(new AppError("RESOURCE_NOT_FOUND", { reason: "Profile not found" }));
    return ResponseHelper.success(res, { profile: updated }, SUCCESS_CODES.OPERATION_SUCCESS);
});
export const deleteMyProfile = asyncHandler(async (req, res, next) => {
    ...
    const updated = await ProfileModel.findOneAndUpdate({
        ...
        schoolId: ctx.schoolId,
        deletedAt: null,
    }, { $set: { deletedAt: new Date() } }, { new: true }).lean();
    if (!updated)
        return next(new AppError("RESOURCE_NOT_FOUND", { reason: "Profile not found" }));
    return ResponseHelper.success(res, { profile: updated }, SUCCESS_CODES.OPERATION_SUCCESS);
});
export const changeMyPassword = asyncHandler(async (req, res, next) => {
    ...
    if (!ok)
        return next(new AppError("ACCESS_DENIED", { message: "Invalid current password" }));
    user.password = newPassword; // pre-save hook hashes it
    user.passwordChangedAt = new Date();
    // Optional: bump tokenVersion so existing tokens become invalid
    user.tokenVersion = (user.tokenVersion || 0) + 1;
    await user.save();
    return ResponseHelper.success(res, { message: "Password updated" }, SUCCESS_CODES.OPERATION_SUCCESS);
});
