...
export const createAbsentNotifications = asyncHandler(async (req, res) => {
    ...
    if (attendance.isHoliday) {
        return ResponseHelper.success(res, { created: 0, skipped: [], reason: "Holiday" }, SUCCESS_CODES.OPERATION_SUCCESS);
    }
    const absentStudentIds = (attendance.entries || [])
        .filter((e) => e.status === "absent")
        .map((e) => new Types.ObjectId(String(e.studentId)));
    ...
    const students = await StudentProfile.find({
        ...
        .lean();
    const phoneById = new Map(students.map((s) => {
        const u = s.userId || {};
        const fullName = [u.firstName, u.lastName].filter(Boolean).join(" ").trim();
        return [
            String(s._id),
            { phone: s.guardianNumber, name: s.guardianName, fullName },
        ];
    }));
    ...
    if (candidates.length === 0) {
        return ResponseHelper.success(res, { created: 0, skipped: absentStudentIds.map(String), reason: "No guardian phone numbers" }, SUCCESS_CODES.OPERATION_SUCCESS);
    }
...
export const listAbsentNotifications = asyncHandler(async (req, res) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
        throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    }
    ...
    const dateLocalRaw = req.query.dateLocal?.trim() || req.query.date?.trim();
    ...
    const notifFilter = { schoolId: ctx.schoolId };
    if (dateLocalRaw) {
    ...
    if (needStudentScope) {
        const studentQuery = { schoolId: ctx.schoolId, deletedAt: null };
        ...
        if (sessionIdRaw) {
            ...
            const classFilter = { schoolId: ctx.schoolId, deletedAt: null, academicSessionId: new Types.ObjectId(sessionIdRaw) };
            ...
            if (classIds.length === 0) {
                return ResponseHelper.success(res, { data: [], meta: { page, limit, total: 0, count: 0, pages: 1 } }, SUCCESS_CODES.DATA_FETCHED);
            }
    ...
    return ResponseHelper.success(res, { data: items, meta: { page, limit, total, count: items.length, pages: Math.ceil(total / limit) || 1 } }, SUCCESS_CODES.DATA_FETCHED);
});
