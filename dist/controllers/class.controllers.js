...
export const createClass = asyncHandler(async (req, res) => {
    ...
    const cls = await Class.findById(created._id)
        .select("name level section classTeacherId createdAt updatedAt")
        .lean();
    return ResponseHelper.success(res, { class: cls }, SUCCESS_CODES.CLASS_CREATED);
});
...
export const getAllClasses = asyncHandler(async (req, res) => {
    ...
    // Optional filters
    const currentRaw = req.query.current?.trim();
    const page = Math.max(parseInt(String(req.query.page ?? "1"), 10) || 1, 1);
    const limitRaw = Math.max(parseInt(String(req.query.limit ?? "20"), 10) || 20, 1);
    const limit = Math.min(limitRaw, 100);
    const skip = (page - 1) * limit;
    const sort = req.query.sort?.trim() || "level section"; // stable default
...
export const updateClass = asyncHandler(async (req, res) => {
    ...
    const result = await Class.findById(saved._id)
        .select("name level section classTeacherId createdAt updatedAt")
        .lean();
    return ResponseHelper.success(res, { class: result }, SUCCESS_CODES.CLASS_UPDATED);
});
...
export const deleteClass = asyncHandler(async (req, res) => {
    ...
    const updated = await Class.findByIdAndUpdate(cls._id, { $set: { deletedAt: new Date() } }, { new: true })
...
export const getStudentsOfClass = asyncHandler(async (req, res) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
        throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    }
    ...
    const [items, total] = await Promise.all([
        StudentProfile.find(profileFilter)
            ...
            path: "userId",
            match: userFilter,
            select: "firstName lastName username email phone profileImage dob gender address role isActive",
        })
