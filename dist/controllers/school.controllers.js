...
export const createSchool = asyncHandler(async (req, res) => {
    const { name, slug, udisecode, address, city, state, country, pincode, contactEmail, contactPhone, logoUrl, plan, subscriptionStatus, } = req.body;
    // Uniqueness checks
    ...
    const school = await School.findById(created._id)
        .select("name slug udisecode address city state country pincode contactEmail contactPhone logoUrl isActive plan subscriptionStatus subscriptionStart subscriptionEnd isVerified createdAt updatedAt")
        .lean();
    return ResponseHelper.success(res, { school }, SUCCESS_CODES.SCHOOL_CREATED);
});
...
export const updateSchool = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const ctxUser = req.context?.user;
    if (!id || !Types.ObjectId.isValid(id)) {
        throw new AppError("VALIDATION_ERROR", { reason: "Invalid school id" });
    }
