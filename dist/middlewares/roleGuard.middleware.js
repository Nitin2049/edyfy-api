...
export const roleGuard = (allowed) => {
    ...
    return (req, res, next) => {
        ...
        if (!allowed.includes(ctxUser.role)) {
            ...
            return next(new AppError("ACCESS_DENIED", { reason: "Insufficient role" }));
        }
        next();
    };
};
// Convenience overload: roleGuard("teacher","admin")
export const allow = (...roles) => roleGuard(roles);
...
export const requireAdminOrClassTeacher = async (req, res, next) => {
    try {
        const ctxUser = req.context?.user;
        if (!ctxUser) {
            return next(new AppError("ACCESS_DENIED", { reason: "Unauthenticated" }));
        ...
        if (!teacher) {
            return next(new AppError("ACCESS_DENIED", { reason: "Teacher profile not found" }));
        }
        // Check if this teacher is the assigned class teacher for the classId
        const isOwner = (await teacher.isClassTeacher?.(new Types.ObjectId(classId))) ?? false;
        if (!isOwner) {
            return next(new AppError("ACCESS_DENIED", {
                reason: "Only the assigned class teacher can perform this action for this class",
            }));
        }
        return next();
    }
