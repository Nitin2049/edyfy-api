...
import { registerStudent, listStudentsByClass, getStudentByUsername, updateStudent, softDeleteStudent } from "../controllers/student.controllers.js";
...
router.get("/", auth, attachClassIdFromQuery, requireAdminOrClassTeacher, listStudentsByClass);
// Create a student (admin or that class's class teacher)
router.post("/", auth, validate(createStudentSchema), requireAdminOrClassTeacher, registerStudent);
// Get specific student in a class (admin or that class's class teacher)
// Client must provide  :username and ?classId=<ObjectId>
router.get("/:username", auth, attachClassIdFromQuery, requireAdminOrClassTeacher, getStudentByUsername);
...
router.patch("/:username", auth, validate(updateStudentSchema), attachClassIdFromQuery, requireAdminOrClassTeacher, updateStudent);
// Delete (soft delete) specific student's profile in a class (admin-only)
// Client must provide  :username and ?classId=<ObjectId> and
router.delete("/:username", auth, attachClassIdFromQuery, allow("admin"), softDeleteStudent);
