...
router.patch("/:classId", auth, allow("admin"), validate(classUpdateSchema), updateClass);
// DELETE /api/v1/classes/:classId - Delete (soft delete) a class (admin-only)
router.delete("/:classId", auth, allow("admin"), deleteClass);
// GET /api/v1/classes/:classId/students - List students of a class (admin or class teacher)
router.get("/:classId/students", auth, requireAdminOrClassTeacher, getStudentsOfClass);
