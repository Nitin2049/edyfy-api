...
export const createAcademicSession = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { year, startDate, endDate } = req.body as any;
...
export const getAcademicSessions = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    ...
    const sort = (req.query.sort as string | undefined)?.trim() || "-startDate";

    const q = (req.query.q as string | undefined)?.trim(); // search by year
    const isActiveRaw = (req.query.isActive as string | undefined)?.trim();
...
export const getAcademicSessionById = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
...
export const updateAcademicSession = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
...
export const setCurrentAcademicSession = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    ...
    await AcademicSession.updateMany(
      { schoolId: ctx.schoolId, deletedAt: null, isActive: true },
      { $set: { isActive: false } }
    );
    // Set requested session as active
    const updated = await AcademicSession.findByIdAndUpdate(
      session._id,
      { $set: { isActive: true } },
      { new: true }
    )
...
export const getCurrentAcademicSession = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
...
export const deleteAcademicSession = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
