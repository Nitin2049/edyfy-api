...
import type { AcademicReportCreateInput } from "../validators/academicReportCreate.validator.js";
import type { AcademicReportUpdateInput } from "../validators/academicReportUpdate.validator.js";

export const createAcademicReport = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const body = req.body as AcademicReportCreateInput;
    ...
    const report = await AcademicReport.findById(created._id)
      .select(
        "studentId academicSessionId examName subjects coScholastic remarks markedBy createdAt updatedAt"
      )
      .lean();
...
export const listAcademicReports = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    ...
    const studentIdRaw = (req.query.studentId as string | undefined)?.trim();
    const sessionIdRaw = (req.query.sessionId as string | undefined)?.trim();
    const examName = (req.query.examName as string | undefined)?.trim();
...
export const getAcademicReportById = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    ...
    if (ctx.role === "teacher") {
      ...
      const isOwner =
        (await teacherDoc?.isClassTeacher?.(
          new Types.ObjectId(String(student.classId))
        )) ?? false;
      if (!isOwner) {
        throw new AppError("ACCESS_DENIED", {
...
export const updateAcademicReport = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
    ...
    const body = req.body as AcademicReportUpdateInput;
    ...
    if (
      body.examName &&
      body.examName.trim() &&
      body.examName.trim() !== (report as any).examName
    ) {
      const exists = await AcademicReport.findOne({
        _id: { $ne: report._id },
        schoolId: ctx.schoolId,
      ...
      (report as any).examName = body.examName.trim();
    }

    if (body.subjects) {
      (report as any).subjects = body.subjects as any;
    }
    if (body.coScholastic) {
      (report as any).coScholastic = body.coScholastic as any;
    }
    if (typeof body.remarks === "string") {
      (report as any).remarks = body.remarks;
    }
...
export const deleteAcademicReport = asyncHandler(
  async (req: Request, res: Response) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      throw new AppError("ACCESS_DENIED", { reason: "Unauthenticated" });
