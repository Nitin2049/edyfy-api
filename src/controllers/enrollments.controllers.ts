...
export const createEnrollment = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { classId, studentId, sessionId, notes } = req.body as any;
    ...
    const enrollment = await Enrollment.findById(created._id)
      .select(
        "studentProfileId classId academicSessionId enrollmentDate status notes createdAt updatedAt"
      )
      .lean();
    ...
    if (ctx.role === "teacher") {
      ...
      const isOwner =
        (await teacherDoc?.isClassTeacher?.(
          new Types.ObjectId(String(enrollment.classId))
        )) ?? false;
      if (!isOwner) {
        throw new AppError("ACCESS_DENIED", {
...
export const updateEnrollment = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const enrollmentId = (req.params as any)?.id as string | undefined;
    ...
    const { classId, status, notes } = req.body as any;
    ...
    if (typeof status === "string") {
      const prev = (enrollment as any).status;
      ...
      if (status === "promoted") {
        ...
        if (ctx.role === "teacher") {
          const teacher = await TeacherProfile.findOne({
            ...
            .select("_id")
            .lean();
          enrollment.promotedBy = teacher?._id ?? (undefined as any);
        } else {
          enrollment.promotedBy = undefined as any;
        }
        enrollment.promotedOn = new Date();
      } else if (prev === "promoted" && status !== "promoted") {
        // Clear promoted metadata if moving away from promoted
        enrollment.promotedBy = undefined as any;
        enrollment.promotedOn = undefined as any;
      }
    }
    ...
    if (classId && saved.status === "active") {
      await StudentProfile.updateOne(
        { _id: saved.studentProfileId },
        { $set: { classId: saved.classId } }
      );
    }

    const result = await Enrollment.findById(saved._id)
      .select(
        "studentProfileId classId academicSessionId enrollmentDate status notes promotedBy promotedOn createdAt updatedAt"
      )
      .lean();
