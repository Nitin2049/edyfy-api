  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const exists = await Enrollment.findOne({
        studentProfileId: studentProfile[0]._id as any,
...
export const listStudentsByClass = asyncHandler(
  async (req: Request, res: Response) => {
    const schoolId = req.context?.user?.schoolId;
    if (!schoolId) {
      throw new AppError("ACCESS_DENIED", {
    ...
    const classId = (req.params as any)?.classId || (req.query as any)?.classId;
    if (!classId) {
    ...
    const [items, total] = await Promise.all([
      StudentProfile.find(profileFilter)
        .populate({
          ...
          match: userFilter,
          select:
            "firstName lastName email phone profileImage dob gender address role username isActive",
        })
        .select(
...
export const getStudentByUsername = asyncHandler(
  async (req: Request, res: Response) => {
    const schoolId = req.context?.user?.schoolId;
    if (!schoolId)
      throw new AppError("ACCESS_DENIED", {
    ...
    const rawUsername = (req.params as any)?.username;
    ...
    let userDoc: any = await User.findOne({
      ...
      .select(
        "firstName lastName username email phone profileImage dob gender address role isActive createdAt updatedAt"
      )
      .lean();
    ...
    return ResponseHelper.success(
      res,
      { user: userDoc, profile: profileDoc },
      SUCCESS_CODES.DATA_FETCHED
    );
  }
);
...
export const updateStudent = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    for (const key of allowedUserKeys) {
      if (key in body) userUpdates[key] = body[key];
    }
    ...
    const [updatedUser, updatedProfile] = await Promise.all([
      Object.keys(userUpdates).length
        ? User.findByIdAndUpdate(
            ...
            .select(
              "firstName lastName username email phone profileImage dob gender address isActive"
            )
            .lean()
      ...
      Object.keys(profileUpdates).length
        ? StudentProfile.findByIdAndUpdate(
            ...
            .select(
              "admissionNo rollNo guardianName guardianNumber dateOfAdmission dateOfLeaving status"
            )
            .lean()
