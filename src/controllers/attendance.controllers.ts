...
import { AttendanceUpdateInput } from "../validators/attendanceUpdate.validator.js";
...
const parseLocalDay = (input: unknown): Date => {
...
export const listAttendance = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const dateRaw = (req.query.date as string | undefined)?.trim();
    ...
    const sort = (req.query.sort as string | undefined)?.trim() || "-date";
    ...
    const [items, total] = await Promise.all([
      Attendance.find(filter)
        .select(
          "academicSessionId dateLocal date classId entries notes isHoliday markedBy createdAt updatedAt"
        )
        .sort(sort)
        .skip(skip)
        .limit(limit)
    ...
    const projected = items.map((it: any) => {
...
export const createAttendance = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { classId, sessionId, date, isHoliday, notes } = req.body as any;
    ...
    if (String((cls as any).academicSessionId) !== String(session._id)) {
      throw new AppError("VALIDATION_ERROR", {
    ...
    if (!isHoliday) {
      ...
      for (const e of rawEntries) {
        if (
          !Types.ObjectId.isValid(e.studentId) ||
    ...
    let markedBy: Types.ObjectId | undefined = undefined;
    if (ctx.role === "teacher") {
...
export const updateAttendance = asyncHandler(async (req: Request, res: Response) => {
  ...
  const body = req.body as AttendanceUpdateInput;
  ...
  let targetClassId = attn.classId as any as Types.ObjectId;
  let targetSessionId = attn.academicSessionId as any as Types.ObjectId;
  let targetDate = attn.date;
  let targetDateLocal = (attn as any).dateLocal || formatLocalYYYYMMDD(attn.date);
  ...
  if (body.classId) {
    ...
    targetSessionId = new Types.ObjectId(String((cls as any).academicSessionId));
  }
  ...
  if (body.classId || body.sessionId) {
    ...
    if (String((cls as any).academicSessionId) !== String(targetSessionId)) {
  ...
  if (body.date) {
    ...
    targetDate = parseLocalDay(body.date as any);
  }

  // Enforce uniqueness for the new combination if any of the unique keys changed
  const uniqueChanged =
    String(targetClassId) !== String(attn.classId) ||
    String(targetSessionId) !== String(attn.academicSessionId) ||
    targetDateLocal !== (attn as any).dateLocal || +new Date(targetDate) !== +new Date(attn.date);
  ...
  attn.academicSessionId = targetSessionId as any;
  (attn as any).classId = targetClassId as any;
  (attn as any).dateLocal = targetDateLocal;
  attn.date = targetDate;
  if (typeof body.isHoliday === "boolean") {
    (attn as any).isHoliday = body.isHoliday;
  }
  if (typeof body.notes === "string") {
    (attn as any).notes = body.notes;
  }

  // Entries handling
  if ((attn as any).isHoliday) {
    (attn as any).entries = [];
  } else {
    ...
    if (body.entries) {
      ...
      const nextEntries = body.entries.map((e) => {
        ...
        return { studentId: new Types.ObjectId(e.studentId), status: e.status } as any;
      });
      (attn as any).entries = nextEntries as any;
    }
    ...
    if (body.studentId && body.status) {
      ...
      const entries = ((attn as any).entries || []) as Array<{ studentId: Types.ObjectId; status: string }>;
      ...
      (attn as any).entries = entries as any;
    }
  }
...
export const deleteAttendance = asyncHandler(async (req: Request, res: Response) => {
  ...
  return ResponseHelper.success(
    res,
    { attendance: { _id: attn._id, deleted: true } },
    SUCCESS_CODES.OPERATION_SUCCESS
  );
});
