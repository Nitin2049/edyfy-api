...
type JwtCustomClaims = {
  sub: string; //_id from user model
  profileId: string; //_id from profile model (teacherProfile/studentProfile/adminProfile)
  schoolId: string;
  role: string;
  tokenVersion: number;
};
...
async function resolveActiveProfile(
...
): Promise<{ profile: any; schoolId: string }> {
  if (role === "admin") {
  ...
  const p = await StudentProfile.findOne({ userId, deletedAt: null })
    .select(
      "_id schoolId admissionNo rollNo classId guardianName guardianNumber dateOfAdmission dateOfLeaving academicReports status"
    )
    .lean();
  if (!p)
    throw new AppError("RESOURCE_NOT_FOUND", {
...
const getEnv = (key: string) => {
  ...
  if (!val) throw new AppError("INTERNAL_SERVER_ERROR");
  return val;
};

const signToken = (
  claims: JwtCustomClaims,
  opts: { expiresIn: string | number; type: "access" | "refresh" }
): string => {
  const secret = (
    opts.type === "access"
  ...
  return jwt.sign(claims as object, secret as any, options as any) as string;
};
...
export const login = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user) return next(new AppError("INVALID_CREDENTIALS"));
      ...
      const { profile, schoolId } = await resolveActiveProfile(
        userId,
...
export const logout = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userId = req.context?.user?.sub as string | undefined;
    if (!userId) {
    ...
    return ResponseHelper.success(
      res,
      { message: "Logged out", discardTokens: true },
      SUCCESS_CODES.OPERATION_SUCCESS
    );
  }
);
