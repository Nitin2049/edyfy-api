...
export const registerTeacher = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    let validatedClassId: Types.ObjectId | undefined;
    if (classId) {
      if (!Types.ObjectId.isValid(classId)) {
        return next(
          new AppError("VALIDATION_ERROR", { reason: "Invalid classId" })
        );
      }
      ...
      if (!cls) {
        return next(
          new AppError("RESOURCE_NOT_FOUND", { reason: "Class not found" })
        );
      }
      if (cls.classTeacherId) {
        return next(
          new AppError("DUPLICATE_ENTRY", { reason: "Class already assigned" })
        );
      }
    ...
    return ResponseHelper.success(
      res,
      { userId: user._id, teacherProfileId: teacherProfile._id },
      SUCCESS_CODES.TEACHER_CREATED
    );
  }
);
...
export const getAllTeachers = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const status = (req.query.status as string | undefined)?.trim(); // "active" | "resigned"
    const q = (req.query.q as string | undefined)?.trim(); // search term for name/username/email/phone
    const sort = (req.query.sort as string | undefined)?.trim() || "-createdAt"; // "name", "-createdAt", etc.

    // Build profile filter
    const profileFilter: any = { schoolId: ctx.schoolId, deletedAt: null };
    ...
    const [items, total] = await Promise.all([
      TeacherProfile.find(profileFilter)
        .populate({
          ...
          match: userFilter,
          select:
            "firstName lastName username email address phone profileImage dob gender isActive role",
        })
...
export const getOneTeacher = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      return next(new AppError("ACCESS_DENIED", { reason: "Unauthenticated" }));
    ...
    const raw =
      (req.params as any).username ||
      (req.query as any).username ||
      (req.params as any).teacherUserId ||
      (req.query as any).teacherUserId;

    const username = typeof raw === "string" ? raw.trim().toLowerCase() : "";

    if (!username) {
      return next(
        new AppError("VALIDATION_ERROR", { reason: "username is required" })
      );
    }
    ...
    let user = await User.findOne({
      ...
      .select(
        "firstName lastName username email phone profileImage dob gender isActive role address createdAt updatedAt"
      )
      .lean();
...
export const updateTeacher = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) {
      return next(
        new AppError("RESOURCE_NOT_FOUND", { reason: "Teacher not found" })
      );
    }
    ...
    if (profileImage !== undefined) userSet.profileImage = profileImage;
    ...
    if (qualification !== undefined) profileSet.qualification = qualification;
    if (joinDate !== undefined) profileSet.joinDate = joinDate;
    if (exitDate !== undefined) profileSet.exitDate = exitDate;
    if (status !== undefined) profileSet.status = status;
    ...
    if (classId !== undefined) {
      ...
      if (!existingProfile) {
        return next(
          new AppError("RESOURCE_NOT_FOUND", { reason: "Teacher profile not found" })
        );
      }
      ...
      const oldClassId = existingProfile.classId as any as Types.ObjectId | undefined;
      if (incomingClassId && (!oldClassId || !incomingClassId.equals(oldClassId))) {
        ...
        if (oldClassId) {
          await Class.findOneAndUpdate(
            { _id: oldClassId, classTeacherId: existingProfile._id },
    ...
    updatedProfile = await TeacherProfile.findOneAndUpdate(
      { schoolId: ctx.schoolId, userId: user._id, deletedAt: null },
      Object.keys(profileSet).length ? { $set: profileSet } : {},
      { new: true }
    )
      .select("qualification joinDate exitDate status createdAt updatedAt classId")
      .lean();
...
export const softDeleteTeacher = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const ctx = req.context?.user;
    if (!ctx?.schoolId || !ctx?.role) {
      return next(new AppError("ACCESS_DENIED", { reason: "Unauthenticated" }));
    ...
    if (!profile) {
      return next(
        new AppError("RESOURCE_NOT_FOUND", { reason: "Active teacher profile not found" })
      );
    }
    ...
    if ((profile as any).classId) {
      await Class.findOneAndUpdate(
        { _id: (profile as any).classId, classTeacherId: profile._id },
        { $unset: { classTeacherId: "" } }
      );
    }

    const updated = await TeacherProfile.findOneAndUpdate(
      { schoolId: ctx.schoolId, userId: (user as any)._id, deletedAt: null },
