...
import {
  ...
  allow,
} from "../middlewares/roleGuard.middleware.js";
...
} from "../controllers/academicReport.controllers.js";
...
const attachClassIdFromReport = async (
  ...
  try {
    ...
    const student = await StudentProfile.findOne({
      ...
      .select("classId")
      .lean();
    if (student?.classId) (req.params as any).classId = String(student.classId);
...
router.patch(
  "/:id",
  auth,
  attachClassIdFromReport,
  validate(academicReportUpdateSchema),
  requireAdminOrClassTeacher,
  updateAcademicReport
);
